...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Fetch doctor data for AuthorBox
  const { data: siteConfig } = await supabaseAdmin
    .from("site_config")
    .select("doctor_name, doctor_crm, doctor_rqe, specialty, subspecialty, bio, profile_photo_url")
    .single();
  ...
  const jsonLd = {
    ...
    "author": {
      "@type": "Physician",
      "name": article.author?.name || siteConfig?.doctor_name || "Dr. Pedro Felipe",
    ...
    <div className="bg-[#f8fafc] min-h-screen pb-20">
      ...
      <main className="w-full py-12 md:py-20">
        <article className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <div className="mb-10 text-center md:text-left">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-slate-500 font-medium">
              <time dateTime={article.published_at}>
                Atualizado em {new Date(article.updated_at || article.published_at).toLocaleDateString("pt-BR")}
              </time>
          ...
          <AuthorBox 
            name={siteConfig?.doctor_name || article.author?.name || "Dr. Pedro Felipe"}
            role={`${siteConfig?.specialty || "Cardiologista"} ${siteConfig?.subspecialty ? `e ${siteConfig.subspecialty}` : ""}`}
            crm={`CRM ${siteConfig?.doctor_crm || ""} ${siteConfig?.doctor_rqe?.length ? `/ RQE ${siteConfig.doctor_rqe[0]}` : ""}`}
            bio={siteConfig?.bio || "Especialista dedicado a traduzir a medicina complexa em prevenção prática para o dia a dia."}
            photoUrl={siteConfig?.profile_photo_url}
          />
          ...
          {/* AdSense Intercalado (Simulado no final do conteúdo para não quebrar HTML bruto, mas pode ser injetado no meio) */}
          <AdSenseUnit slot="middle_article" />

          {/* BLOCO DE AFILIADOS / INDICAÇÃO TÉCNICA */}
          <AffiliateBox 
      ...
      <section id="agendamento" className="max-w-5xl mx-auto px-4 sm:px-6 mb-20">
        <div className="bg-gradient-to-br from-[#0284c7] to-[#0369a1] rounded-[2.5rem] p-10 md:p-16 text-center shadow-[0_20px_50px_rgb(2,132,199,0.25)] relative overflow-hidden">
          ...
          <p className="text-sky-100 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed relative z-10">
            Não deixe sua saúde para depois. Agende uma avaliação cardiológica completa presencialmente ou via Telemedicina para todo o Brasil.
          </p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 relative z-10">
            <Link href="/agendar" className="w-full sm:w-auto bg-white text-[#0369a1] hover:bg-slate-50 px-8 py-4 rounded-full font-extrabold text-lg transition-all shadow-lg hover:shadow-xl flex items-center justify-center gap-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
              Agendar Consulta
            </Link>
            <a href="https://wa.me/5561999999999" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto bg-transparent border-2 border-sky-300 text-white hover:bg-sky-800/50 px-8 py-4 rounded-full font-bold text-lg transition-all flex items-center justify-center gap-2">
